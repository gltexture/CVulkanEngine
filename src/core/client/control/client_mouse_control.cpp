...
namespace cvulkan::client::control {
    void MouseControl::glfwCallbacks() {
        glfwSetWindowUserPointer(this->_glfwWindow.glfwWindowDescriptor(), this);
        glfwSetCursorPosCallback(this->_glfwWindow.glfwWindowDescriptor(), [](GLFWwindow* window, const double x, const double y) {
        ...
        glfwSetCursorEnterCallback(this->_glfwWindow.glfwWindowDescriptor(), [](GLFWwindow* window, const int entered) {
            auto* mouse = static_cast<MouseControl*>(glfwGetWindowUserPointer(window));
            bool& inWin = mouse->m_in_window;
            inWin = entered;
        });
        glfwSetMouseButtonCallback(this->_glfwWindow.glfwWindowDescriptor(), [](GLFWwindow* window, const int button, const int action, const int mods) {
            auto* mouse = static_cast<MouseControl*>(glfwGetWindowUserPointer(window));
