...
namespace cvulkan::client::render::core {
    void CVulkanPipelineCache::createPipelineCache() {
        ...
        utility::vkCheck(vkCreatePipelineCache(this->_context.device().vkDevice, &pipelineCacheCreateInfo, nullptr, &this->_vkPipeline), "Error creating vkPipeline cache");
        logging::info("Created vulkan pipeline cache");
    }

    void CVulkanPipelineCache::destroyPipelineCache() const {
        vkDestroyPipelineCache(this->_context.device().vkDevice, this->_vkPipeline, nullptr);
        logging::info("Destroyed vulkan vkPipeline cache");
    }
    ...
    void CVulkanPipeline::createPipeline(const CVulkanPipelineBuildInfo& buildInfo) {
        ...
        utility::vkCheck(vkCreateGraphicsPipelines(this->_context.device().vkDevice, this->_context.pipelineCache().vkPipeline(), 1, &createInfo, nullptr, &this->_vkPipeline), "Error creating graphics pipeline");
    }
